<?php
    ...
    if(!empty(extract($_POST))){
        ...
        $query_rows_count = mysqli_num_rows(mysqli_query($conn,"SELECT * FROM queries_doubts WHERE user_id=".$_POST["sid"]." AND user_type='teacher' AND user_doubt='$message'"));
        if($query_rows_count == 0){
            if(mysqli_query($conn,"INSERT INTO queries_doubts(user_id, user_gi_pin, user_type, user_email, user_doubt) VALUES(".$_POST["sid"].", ".$_POST["sgr"].", 'teacher', '$email', '$message')")){
...
<html>
        ...
        <script>
            function ajax_call()
            {
                $.ajax({
                    url:"Addstudent.php",
                    method:"post",
                    data:{"t_pin":localStorage.getItem("tpin")},
                    dataType:"json",
    ...
    <body class="faqsBody">
        ...
        <div class="outer_faqs7">
            <div class="faqs7">
                <img src="photo/rightArrow.png" class="faqImg7">
                <img src="photo/downArrow.png" class="faqImg07">
                <p class="faqTextQ7">Type out if any other queries / doubts about the website or anything else ?<label style="color: blue;cursor: pointer;">Click Here</label></p>
            </div>
            <div class="outer_faqsTitleInfo7">
                <div class="outer_faqsDaubts">
                    <textarea class="faqsDaubts" id= "Pro-Description" maxlength="800" required></textarea>
                    <label>Description Of The Project</label>
                </div>
                <div class="outer_faqsClearBtn">
                    <input type="button" class="faqsClearBtn" value="CLEAR">
                </div>
                <div class="outer_faqsSendBtn">
                    <input type="button" class="faqsSendBtn" value="SEND" onclick="faqsSendBtn()">
                </div>
            </div>
        </div>
        <!-- ////////////////////////////////////////////////////////////////////////////// -->
        ...
        <script type="text/javascript">
            ...
            function faqsSendBtn(){
                if(($(".faqsDaubts").val()).trim() == ""){
                    wrongVal("Error!", "Your Message Is Empty");
                }else{
                    $.ajax({
                        url:"FAQs.php",
                        type:"POST",
                        dataType:"json",
                        data:{"sid":localStorage.getItem("tid"),"message":($(".faqsDaubts").val()).trim(),"sgr":localStorage.getItem("tpin"),"semail":localStorage.getItem("temail")},
                        success: function(results){
                            console.log(results["send"]);
                            if(results["send"] == "yes"){
                                rightVal("Success!", "Sent Your Querie/Doubt");
                                $("#Pro-Description").val("");
                                $(".outer_faqsDaubts label").css("color", "rgb(196, 187, 187)");
                                $(".faqTextQ7").click();
                                ...
                                wrongVal("Sorry!", "Querie/Doubt Were Not Sent");
                            }
                        },
                        error:function(e){
                            console.log(e);
