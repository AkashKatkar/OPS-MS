<?php
    ...
    if(!empty(extract($_POST)))
    {
        ...
        if($_POST["DelAcc"] == "DelAcc"){
            ...
            $Pass = md5($_POST["pass"]);
            $query_data = mysqli_fetch_array(mysqli_query($conn,"select * from admin where a_id=$userid"));
            if($Pass != $query_data["a_pass"]){
                $return_data["wrongPass"] = "yes";
            }else if(mysqli_query($conn, "UPDATE admin SET a_type='deactivate sudo_user' WHERE a_id=$userid")){
            ...
            $userid = $_POST["uid"];
            if(mysqli_query($conn, "UPDATE admin SET a_name='$username',a_email='$useremail',a_pn='$usernumber' WHERE a_id=$userid")){
            ...
            $otp=rand(1000,9999);
            $msg="<html><body><label style='font-size:1.5em'>Hello <label style='color:orange'>".$admin_name."</label>, One Time Password(OTP) is <h3 style='display:inline-block'>".$otp."</h3></label></body></html>";
            $headers = "MIME-Version: 1.0" . "\r\n";
            $headers .= "Content-type:text/html;charset=UTF-8" . "\r\n";
            $headers .= 'From: PROJECT MANAGEMENT SYSTEM<cartoonlovers185@gmail.com>';
            if(mail($admin_email,"User Verification",$msg,$headers)){
            ...
            $query_data = mysqli_fetch_array(mysqli_query($conn,"select * from admin where a_id=$admin_id"));
            if($query_data["a_pass"] == md5($currPass)){
                $return_data["verifyPass"] = "yes";
            }else{
                $return_data["verifyPass"] = "no";
            }
        }else if($_POST["DelAcc"] == "saveNewPass"){
            $admin_pass = md5($_POST["upass"]);
            $admin_id = $_POST["uid"];
            date_default_timezone_set('ASIA/KOLKATA');
            $current_date = (date("Y-m-d") ." ". date("H:i:s"));
            $query_data = mysqli_fetch_array(mysqli_query($conn,"select * from admin where a_id=$admin_id"));
            if($admin_pass==$query_data['a_pass']){
                $return_data["exist"] = "yes";
            }else if(mysqli_query($conn, "UPDATE admin SET a_pass='$admin_pass', a_email_time='$current_date' WHERE a_id=$admin_id")){
            ...
            $result1=mysqli_query($conn,"SELECT * FROM admin WHERE a_email='$useremail' AND a_id!=$userid AND a_type!='deactivate sudo_user'");
            $query_row1 = mysqli_fetch_array($result1);
            $result2=mysqli_query($conn,"SELECT * FROM admin WHERE a_pn='$usernumber' AND a_id!=$userid AND a_type!='deactivate sudo_user'");
            $query_row2 = mysqli_fetch_array($result2);
            if((mysqli_num_rows($result1)>0 || mysqli_num_rows($result2)>0)){
                if($query_row2["a_pn"] == $usernumber && $query_row1["a_email"] == $useremail){
                    $return_data["savedetails"] = "existnumemail";
                }else if($query_row2["a_pn"] == $usernumber){
                    $return_data["savedetails"] = "existnum";
                }else if($query_row1["a_email"] == $useremail){
...
<html>
        ...
        <script>
            function ajax_call()
            {
                $.ajax({
                    url:"Addstudent.php",
                    method:"post",
                    data:{"a_id":localStorage.getItem("aid")},
                    dataType:"json",
                    success:function(resp){
                        if(resp["a_type"]=="deactivate sudo_user")
                        {
    ...
    <body class="ASBody">
        ...
        <div class="outer-OriginalPasswordBox">
            <div class="originalPasswordBox">
                ...
                <div class="outer_opbVerifyBtn">
                    <input type="button" class="opbVerifyBtn opbVerifyBtnp1" onclick="deleteacc(localStorage.getItem('aid'))" value="VERIFY">
        ...
        <script type="text/javascript">
            $(".pDFirstNameField").val(localStorage.getItem("aname"));
            $(".pDNumberField").val(localStorage.getItem("apn"));
            $(".pDEmailIdField").val(localStorage.getItem("aemail"));
                    ...
                    jQuery.ajax({
                        ...
                        success: function(results){
                            if(results["wrongPass"] == "yes"){
                                ...
                                localStorage.removeItem("apn");
                                setTimeout(function(){
                                    location.href = "Admin%20Module/AdminLogin.php";
                                },3500);
                            }else{
