<?php
    ...
    function admin_login(){
        ...
        if(!$conn1){
            ...
            $result1=mysqli_query($conn1,"Select * from admin where a_id=$id");
            $row=mysqli_fetch_array($result1);
            if(mysqli_num_rows($result1)>0 && $row["a_type"] != "deactivate sudo_user"){
                if($row["a_pass"]!=""){
                    if($row["a_pass"]==md5($pass)){
                        $return_data["aid"]=$row["a_id"];
                        $return_data["aname"]=$row["a_name"];
                        $return_data["atype"]=$row["a_type"];
                        $return_data["aemail"]=$row["a_email"];
                        $return_data["apn"]=$row["a_pn"];
                    }else{
