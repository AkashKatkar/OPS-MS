<?php
    ...
    if(!empty(extract($_POST)))
    {
        ...
        if($_POST["DelAcc"] == "DelAcc"){
            ...
            $Pass = md5($_POST["pass"]);
            $query_data = mysqli_fetch_array(mysqli_query($conn,"select * from student where s_id=$userid"));
            if($Pass != $query_data["s_pass"]){
                $return_data["wrongPass"] = "yes";
            }else if(mysqli_query($conn, "UPDATE student SET s_pass=NULL WHERE s_id=$userid")){
            ...
            $userid = $_POST["uid"];
            if(mysqli_query($conn, "UPDATE student SET s_name='$username',s_email='$useremail',s_pn='$usernumber' WHERE s_id=$userid")){
            ...
            $student_email = $_POST["uemail"];
            $otp=rand(1000,9999);
            $msg="<html><body><label style='font-size:1.5em'>Hello <label style='color:orange'>".$student_name."</label>, One Time Password(OTP) is <h3 style='display:inline-block'>".$otp."</h3></label></body></html>";
            $headers = "MIME-Version: 1.0" . "\r\n";
            $headers .= "Content-type:text/html;charset=UTF-8" . "\r\n";
            $headers .= 'From: PROJECT MANAGEMENT SYSTEM<cartoonlovers185@gmail.com>';
            if(mail($student_email,"User Verification",$msg,$headers)){
            ...
            $query_data = mysqli_fetch_array(mysqli_query($conn,"select * from student where s_id=$student_id"));
            if($query_data["s_pass"] == md5($currPass)){
                $return_data["verifyPass"] = "yes";
            }else{
                $return_data["verifyPass"] = "no";
            }
        }else if($_POST["DelAcc"] == "saveNewPass"){
            $student_pass = md5($_POST["upass"]);
            $student_id = $_POST["uid"];
            date_default_timezone_set('ASIA/KOLKATA');
            $current_date = (date("Y-m-d") ." ". date("H:i:s"));
            $query_data = mysqli_fetch_array(mysqli_query($conn,"select * from student where s_id=$student_id"));
            if($student_pass==$query_data['s_pass']){
                $return_data["exist"] = "yes";
            }else if(mysqli_query($conn, "UPDATE student SET s_pass='$student_pass', s_email_time='$current_date' WHERE s_id=$student_id")){
            ...
            $result1=mysqli_query($conn,"SELECT * FROM student WHERE s_email='$useremail' AND s_id!=$userid");
            $query_row1 = mysqli_fetch_array($result1);
            $result2=mysqli_query($conn,"SELECT * FROM student WHERE s_pn='$usernumber' AND s_id!=$userid");
            $query_row2 = mysqli_fetch_array($result2);
            if((mysqli_num_rows($result1)>0 || mysqli_num_rows($result2)>0)){
                if($query_row2["s_pn"] == $usernumber && $query_row1["s_email"] == $useremail){
                    $return_data["savedetails"] = "existnumemail";
                }else if($query_row2["s_pn"] == $usernumber){
                    $return_data["savedetails"] = "existnum";
                }else if($query_row1["s_email"] == $useremail){
...
<html>
    ...
    <body class="ASBody">
        ...
        <div class="outer_deaAProceedBox">
            <div class="deaAProceedBox">
                ...
                <div class="outer_aAAPDeleteBtn">
                    <input type="button" class="aAAPDeleteBtn" value="DELETE" onclick="deleteAcco(localStorage.getItem('aid'))">
                </div>
            </div>
        </div>
        <!-- ////////////////////////////////////////////////////////////////////////////////////// -->
        ...
        <div class="outer-OriginalPasswordBox">
            <div class="originalPasswordBox">
                ...
                <div class="outer_opbVerifyBtn">
                    <input type="button" class="opbVerifyBtn opbVerifyBtnp1" onclick="deleteacc(localStorage.getItem('sid'))" value="VERIFY">
        ...
        <script type="text/javascript">
            $(".pDFirstNameField").val(localStorage.getItem("sname"));
            $(".pDNumberField").val(localStorage.getItem("spn"));
            $(".pDEmailIdField").val(localStorage.getItem("semail"));
                    ...
                    jQuery.ajax({
                        ...
                        success: function(results){
                            if(results["wrongPass"] == "yes"){
                                ...
                                localStorage.removeItem("spn");
                                setTimeout(function(){
                                    location.href = "http://localhost/MAINFILE8/Student%20Module/StudentLogin.php";
                                },3500);
                            }else{
                                wrongVal("Sorry!", "Your Account Is Not Deleted, Please Try Again");
                            }
                        },
                        error:function(e){
                            console.log(e);
